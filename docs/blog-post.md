...
Claude Code 有 Hook 机制，社区也有人做过通知插件。但有个坑：
...
- **VS Code 在运行** → 扩展监听事件并发送系统通知（支持点击跳转到 VS Code 窗口或终端、延迟、去重等）
- **纯 CLI 模式** → 自动回退，直接调用 `terminal-notifier` 或 `osascript` 发送通知，点击可聚焦到终端窗口

## 三步安装

1. **安装插件** — VS Code 扩展商店搜索 **"Claude Code Notifier Plus"**

2. **安装 terminal-notifier**（推荐，macOS）
...
3. **授权辅助功能** — 系统设置 → 隐私与安全性 → 辅助功能 → 开启 terminal-notifier（一次性）
