...
function showOsNotification(text, { clickToFocus = 'window', project = '', cwd = '', iconPath = '', termBundleId = '' } = {}) {
  ...
  if (platform === 'darwin') {
    ...
    if (tnPath) {
      const groupId = `claude-notif-${process.pid}-${++notifCounter}`;
      const removeCmd = `${tnPath} -remove '${groupId}'`;
      ...
      if (clickToFocus === 'window') {
        if (termBundleId) {
          executeScript = buildTerminalActivateScript(termBundleId, project) || '';
        } else {
          executeScript = buildActivateScript(project);
        }
      }
      args.push('-execute', executeScript ? `${executeScript}; ${removeCmd}` : removeCmd);
